...
<html>
    <head>
        ...
        <% } %>
        <title>Workforce Analysis</title>
    </head>
    <body>
        <div style="-fs-page-sequence: start; page-break-before: avoid">
            ...
            <% if (!params.pdf) { %>            
            <div class="icons">
                <g:link controller="pdf" action="show" params="[url:'/reports/workforceAnalysis',sort:params.sort,order:params.order]" class="pdf" title="Export to PDF" />
                <g:link class="xls" title="Export to Excel" params="[excel:'true']" controller="reports" action="workforceAnalysis" />
            </div> 
            <% } %>
            <div class="clear"></div>
            <div class="list">
                <g:render  template="workforceAnalysisTable" model="[x:x]"/>
            </div>
